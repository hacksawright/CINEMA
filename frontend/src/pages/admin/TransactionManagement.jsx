...
import { Search, Filter, Download, TrendingUp, DollarSign, Calendar as CalendarIcon, BarChart3 } from "lucide-react";
...
const TransactionManagement = () => {
  ...
  const fetchTransactions = async () => {
      ...
      try {
          ...
          toast({ title: "Lỗi", description: `Không thể tải danh sách giao dịch: ${error.message}`, variant: "destructive"});
      } finally {
          setLoading(false);
      }
  };
  ...
  const filteredTransactions = transactions.filter(transaction => {
    ...
    const matchesSearch = (transaction.id?.toString().toLowerCase().includes(lowerSearchTerm) ||
    ...
    const matchesMethod = methodFilter === "all" || transaction.paymentMethod?.toLowerCase() === methodFilter;
    ...
    if (dateRange?.from && transaction.transactionDate) {
         try {
             ...
             matchesDateRange = transactionDate >= fromDateStart && (!toDateEnd || transactionDate <= toDateEnd);
  ...
  const getStatusBadge = (status) => {
    ...
    switch (lowerStatus) {
      ...
      case "pending": return <Badge className="bg-yellow-600">Chờ xử lý</Badge>;
   ...
   const getMethodBadge = (method) => {
    ...
    const successfulTransactions = filteredTransactions.filter(t => t.status?.toLowerCase() === "success").length;
  ...
  const exportReport = () => {
    if (filteredTransactions.length === 0) {
      toast({ title: "Không có dữ liệu", description: "Không có giao dịch nào để xuất.", variant: "secondary" });
      return;
    }
    ...
    for (const transaction of filteredTransactions) {
      const showtime = transaction.showtimeStartsAt ? format(parseISO(transaction.showtimeStartsAt), 'HH:mm dd/MM/yyyy') : '-';
      const row = [
    ...
    toast({ title: "Đã xuất báo cáo", description: "File CSV đã được tải về." });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card><CardContent className="p-4"><p className="text-sm font-medium text-muted-foreground">Tổng doanh thu</p><p className="text-2xl font-bold text-green-600">{totalRevenue.toLocaleString('vi-VN')} ₫</p></CardContent></Card>
          <Card><CardContent className="p-4"><p className="text-sm font-medium text-muted-foreground">Tổng giao dịch</p><p className="text-2xl font-bold">{totalTransactions}</p></CardContent></Card>
          <Card><CardContent className="p-4"><p className="text-sm font-medium text-muted-foreground">Thành công</p><p className="text-2xl font-bold text-green-600">{successfulTransactions}</p></CardContent></Card>
          <Card><CardContent className="p-4"><p className="text-sm font-medium text-muted-foreground">Tỷ lệ thành công</p><p className="text-2xl font-bold text-blue-600">{successRate}%</p></CardContent></Card>
       </div>

      <Card>
        <CardHeader><CardTitle>Doanh thu theo phương thức thanh toán</CardTitle></CardHeader>
        <CardContent>
          {loading ? <p>Đang tải...</p> : Object.keys(revenueByMethod).length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {Object.entries(revenueByMethod).map(([method, amount]) => (
                  <div key={method} className="p-4 border border-border rounded-lg">
                    ...
                    <p className="text-lg font-bold text-foreground">{amount.toLocaleString('vi-VN')} ₫</p>
                     {totalRevenue > 0 && (
            ...
            <Table>
                ...
                <TableBody>
                   {filteredTransactions.length === 0 ? (
                       <TableRow><TableCell colSpan={8} className="text-center">Không có giao dịch nào khớp.</TableCell></TableRow>
                   ) : (
                         ...
                         <TableRow key={transaction.id}>
                           ...
                           <TableCell>
                             <div>
                               ...
                               <p className="text-xs text-muted-foreground">{formatShortDateTime(transaction.showtimeStartsAt)}</p>
                             </div>
                           </TableCell>
      ...
      <Card>
        <CardHeader><CardTitle>Biểu đồ doanh thu theo ngày (Chưa hoạt động)</CardTitle></CardHeader>
        <CardContent>
