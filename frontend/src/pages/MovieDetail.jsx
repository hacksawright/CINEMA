...
export default function MovieDetail() {
    ...
    try {
        ...
        console.log('Movie data từ API:', movieData);
        console.log('Showtimes từ API:', movieData.showtimes);
        setMovie(movieData);

        const now = new Date();
        const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
        console.log('Thời gian hiện tại:', now);
        console.log('Bắt đầu ngày hôm nay (00:00:00):', new Date(todayStart));
        
        const allShowtimes = movieData.showtimes || [];
        console.log('Tổng số showtimes từ API:', allShowtimes.length);
        
        // Hiển thị tất cả showtime từ ngày hôm nay trở đi (bao gồm cả những showtime đã qua trong ngày hôm nay)
        const validShowtimes = allShowtimes
            .filter(showtime => {
                if (!showtime.startsAt) {
                    console.warn('Showtime không có startsAt:', showtime);
                    return false;
                }
                const showtimeStart = new Date(showtime.startsAt);
                const showtimeStartMs = showtimeStart.getTime();
                
                // Hiển thị showtime từ đầu ngày hôm nay (00:00:00) trở đi
                const isValid = showtimeStartMs >= todayStart;
                
                if (!isValid) {
                    const showtimeDate = new Date(showtimeStartMs);
                    console.log('Showtime bị lọc ra (quá khứ, trước hôm nay):', {
                        showtime: showtimeDate.toLocaleString('vi-VN'),
                        showtimeTimestamp: showtimeDate,
                        todayStart: new Date(todayStart).toLocaleString('vi-VN')
                    });
                } else {
                    console.log('Showtime hợp lệ:', new Date(showtime.startsAt).toLocaleString('vi-VN'));
                }
                return isValid;
            }) 
            .sort((a, b) => new Date(a.startsAt).getTime() - new Date(b.startsAt).getTime());

        console.log('Số showtimes hợp lệ sau khi lọc:', validShowtimes.length);
